...
{
	if(len >= Send_Len)	len = Send_Len;
	mymemcpy(USART2_Send_Buf,buf,len);
	Usart2_Send_Length = len;
	Usart2_Send_Cnt = 0;
...
{  	
	...
	if(USART_GetITStatus(USART2,USART_IT_TXE) != RESET)	//发送中断(发送寄存器为空时)
	{
		if(Usart2_Send_Cnt < Usart2_Send_Length)              
		{
				USART2->DR = USART2_Send_Buf[Usart2_Send_Cnt]&0x01FF;
				Usart2_Send_Cnt++;
		}
		else
		{
