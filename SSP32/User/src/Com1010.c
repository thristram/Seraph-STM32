...
void sicp_send_message(SICP_Message *tx)
{
	u8 payload_len;
...
void sicp_cmd_refresh(void) //刷新SS下ST的环境光颜色和亮度,SC上的电流
{
	...
	for(i = 0; i < 20;i++){
		if((ss.st[i].meshid) && (!ss.st[i].deviceid[0])){//mesh id不为空，则进行点名
			...
			sicp_send_message(&refresh_data);
		}
	}
	for(i = 0; i < 5;i++){
...
void sicp_qe_action_cmd(void)
{
	...
	if(ack_qe){
		...
		if(ss_qe.MDID){//说明快速执行的是SLC或SPC
			if((ss_qe.action[0] == 'D') && (ss_qe.action[0] == 'D')){
				for(i = 0; i < 5;i++){
					for(j = 0; j < 15;j++){
			...
			else if((ss_qe.action[0] == 'W') && (ss_qe.action[0] == 'P')){
				for(i = 0; i < 5;i++){
					for(j = 0; j < 15;j++){
		...
		if((ss_qe.action[0] == 'C') && (ss_qe.action[0] == 'P')){//快速执行的是ST
			for(i = 0; i < 20; i++){
